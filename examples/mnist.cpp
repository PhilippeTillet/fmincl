...
public:
    ...
    void operator()(VectorType const & X, ScalarType & val, VectorType & grad, umintl::value_gradient) {
        set_weights(X);
        feedforward(data_);
...
//    optimization.direction = new umintl::low_memory_quasi_newton<BackendType>(8);
    optimization.direction = new umintl::quasi_newton<BackendType>();

    optimization.stopping_criterion = network.create_early_stopping(testing_data,testing_label);
    optimization.max_iter = 1000;
    optimization.verbosity_level=3;
